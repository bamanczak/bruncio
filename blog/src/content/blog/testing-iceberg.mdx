...
Why is it hard to follow the test pyramid, and what should you do about it?
...
The Test Pyramid is preached in lots of testing sermons and printed on t-shirts worn by hip QA engineers. Yet, in practice, lots of projects end up with struggling while embracing the pyramid in the test strategy 
...
But this doesn't change the fact that as a tester, you need to know how your application is built. Is it a monolith or a bunch of microservices playing hide and seek? Is it lounging in the cloud or hiding in an on-premise server room?
...
I'm not saying that every tester should now engage in "white box testing", re-skill and start writing unit tests (although every tester would learn a lot from trying to do so from time to time). I think it's ok to keep the focus on writing unit tests with developers. 

What I'm trying to say, is that, even if a tester does not write the unit tests, the tester should still know what kind of unit tests are getting written, how do they work, what do they test. This gets closer to "Grey-Box Testing". Every tester should take a sneak peek under the surface of the water. You could see what tests lie there. But you won't be 
able to do that **without proper training and help**. 

Once you invest time and effort, you'll have the full iceberg in sight. Only then can you grasp the entirety of the testing possibilities and decide rightly where to best invest your time.

"Black-Box", "White-Box" and "Grey-Box" testing methods only relate loosely to the test pyramid itself. "Black-Box" testing could be used just as well to writing the e2e tests, as to writing unit tests, so do other methods. However, in my mind, we won't be able to implement a proper test automaton strategy without being able to apply all of those methods.
...
3. Collaborate with developers to get a complete picture of the system.
